...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate_username(self, username):
        ...
        if username.isdigit():
            raise ValidationError(
                {
                    "message": "This username is entiraly numeric"
                }
            )
        if username == validate_password:
            raise ValidationError(
                {
                    "message": "The username must not be equal to the password"
                }
            )
        return username
    ...
    def validate_first_name(self, first_name):
        ...
        if first_name.isdigit():
            raise ValidationError(
                {
                    "message": "This firstname is entiraly numeric"
                }
            )
        return first_name
    ...
    def validate_last_name(self, last_name):
        if len(last_name) < 5 or len(last_name) > 30:
            raise ValidationError(
                {
                    "message": "Lastname must be between 5 and 30 character long"
                }
            )
        if last_name.isdigit():
            raise ValidationError(
                {
                    "message": "This lastname is entiraly numeric"
                }
            )
        return last_name
